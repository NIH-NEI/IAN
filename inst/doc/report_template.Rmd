...
# Extract everything and skip garbage 
if (length(hub_gene_index) > 0) {
  hub_gene_text <- lines[(hub_gene_index+2)]
    cat(hub_gene_text)
}
...
parts <- unlist(strsplit(final_response, "\n"))
print(parts)

similar_systems_index <- grep("^Similar Systems:", parts)
hub_gene_index <- grep("^Hub Gene Identification:", parts)

# Extract the text between the two markers (if both are found)
if (length(similar_systems_index) > 0 && length(hub_gene_index) > 0) {
  # Extract the parts between the Similar Systems and Hub Gene Identification sections
  extracted_parts <- parts[(similar_systems_index + 1):(hub_gene_index - 1)]

  # Combine the extracted parts into a single string
  similar_systems_text <- paste(extracted_parts, collapse = "**") # Rejoin with the delimiter

  # Print the extracted text
  cat(similar_systems_text)
} else {
  similar_systems_text <- ""
  warning("Could not find both Similar Systems and Hub Gene Identification sections.")
}
```


```{r debug_novelty, eval=TRUE}
print("--- Debugging Novelty Chunk ---")
print(paste("Length of parts:", length(parts)))
print("Parts vector:")
print(parts)

novelty_exploration_index <- grep("^Novelty Exploration:", parts)
print(paste("Novelty Exploration Index:", novelty_exploration_index))

step6_index <- grep("^Step 6:", parts)
print(paste("Step 6 Index:", step6_index))
```
...
if (length(novelty_exploration_index) > 0) {
  # Extract the text from the Novelty Exploration section to the end of Step 5
  extracted_parts <- parts[(novelty_exploration_index[1] + 1):(grep("^Step 6:", parts)[1] - 1)]

  # Combine the extracted parts into a single string
  novelty_exploration_text <- paste(extracted_parts, collapse = "\n")

  # Print the extracted text
  cat(novelty_exploration_text)
} else {
  novelty_exploration_text <- ""
...
file_path <- "enrichment_results/final_response.txt" # relative file path
download_link <- paste0("<a href='", file_path, "'>Open full final_response.txt text in new tab</a>")
...
```{r full_response_file, results='asis', message=FALSE, warning=FALSE}
file_path <- "enrichment_results/system_model.txt" # relative file path
download_link <- paste0("<a href='", file_path, "'>Open full system_model.txt text in new tab</a>")
...
plot_filename <- "kegg_enrichment_plot.png"
ggsave(plot_filename, plot = kegg_plot, width = 8, height = 6, dpi = 300)


# Create the download link
download_link <- paste0("<a href='", plot_filename, "' download>Download Plot as PNG</a>")
...
# Save the plot to a file
plot_filename <- "wp_enrichment_plot.png"
...
```{r integrated_network, results='asis', fig.width=8, fig.height=6, message=FALSE, warning=FALSE}
  ...
  full_integrated_network = visualize_system_model(network_revision_response, html_file = "full_integrated_network.html", gene_symbols, output_dir)
...
download_link_model <- paste0("<a href='enrichment_results/system_model.txt'>Click to open system_model.txt in new tab</a>")
download_link_final <- paste0("<a href='enrichment_results/final_response.txt'>Click to open final_response.txt in new tab</a>")
download_link_agents <- paste0("<a href='enrichment_results/agent_responses.txt'>Click to open agent_responses.txt in new tab</a>")
download_link_kegg_original <- paste0("<a href='enrichment_results/kegg_enrichment_original.txt'>Click to open kegg_enrichment_original.txt in new tab</a>")
download_link_kegg_filtered <- paste0("<a href='enrichment_results/kegg_enrichment_filtered.txt'>Click to open kegg_enrichment_filtered.txt in new tab</a>")
download_link_wp_original <- paste0("<a href='enrichment_results/wp_enrichment_original.txt'>Click to open wp_enrichment_original.txt in new tab</a>")
download_link_wp_filtered <- paste0("<a href='enrichment_results/wp_enrichment_filtered.txt'>Click to open wp_enrichment_filtered.txt in new tab</a>")
download_link_reactome_original <- paste0("<a href='enrichment_results/reactome_enrichment_original.txt'>Click to open reactome_enrichment_original.txt in new tab</a>")
download_link_reactome_filtered <- paste0("<a href='enrichment_results/reactome_enrichment_filtered.txt'>Click to open reactome_enrichment_filtered.txt in new tab</a>")
download_link_chea_original <- paste0("<a href='enrichment_results/chea_enrichment_original.txt'>Click to open chea_enrichment_original.txt in new tab</a>")
download_link_chea_filtered <- paste0("<a href='enrichment_results/chea_enrichment_filtered.txt'>Click to open chea_enrichment_filtered.txt in new tab</a>")
download_link_string_original <- paste0("<a href='enrichment_results/string_interactions_original.txt'>Click to open string_interactions_original.txt in new tab</a>")
download_link_string_filtered <- paste0("<a href='enrichment_results/string_interactions_filtered.txt'>Click to open string_interactions_filtered.txt in new tab</a>")
download_link_string_properties <- paste0("<a href='enrichment_results/string_network_properties.txt'>Click to open string_network_properties.txt in new tab</a>")
download_link_pathway_comparison <- paste0("<a href='enrichment_results/pathway_comparison.txt'>Click to open pathway_comparison.txt in new tab</a>")
download_link_integrated_network <- paste0("<a href='enrichment_results/integrated_network.txt'>Click to open integrated_network.txt in new tab</a>")
download_link_deg <- paste0("<a href='enrichment_results/integrated_network.txt'>Click to open integrated_network.txt in new tab</a>")
