...
# Assuming the content will extract to the last bit
# Extract the text from the last bit onwards
extracted_parts <- parts[(novelty_exploration_index + 1):length(parts)]

# Combine the extracted parts into a single string
novelty_exploration_text <- paste(extracted_parts, collapse = "**") # Rejoin with the delimiter
...
```{r integrated_network, results='asis', fig.width=8, fig.height=6, message=FALSE, warning=FALSE}
  ...
  full_integrated_network = visualize_system_model(network_revision_response, html_file = "full_integrated_network.html", gene_symbols)
