...
# Extract the "Potential Upstream Regulators" section
potential_hub_genes <- stringr::str_extract(
  final_response,
  "(?s)(?<=Hub Gene Identification:\\*\\*\\n\\n).*?(?=\\n\\n\\*\\*Drug Target/Marker/Kinase/Ligand Analysis:)"
)

# Check if the extraction was successful
if (is.na(potential_hub_genes)) {
  warning("Could not extract the Potential Upstream Regulators section. Check the format of 'final_response' carefully.")
  potential_hub_genes <- "" # Assign an empty string to avoid further errors
}

# Print the extracted content (for verification)
cat(potential_hub_genes)
...
# Extract the Similar Systems section content
similar_systems <- stringr::str_extract(
  final_response,
  "(?s)(?<=Similar Systems:\\*\\*\\n\\n).*?(?=\\n\\n\\*\\*Hub Gene Identification)"
)

# Check if the extraction was successful
if (is.na(similar_systems)) {
  warning("Could not extract the Similar Systems section. Check the format of 'final_response' carefully.")
  similar_systems <- "" # Assign an empty string to avoid further errors
}
...
# Extract the Novelty Exploration section content
novelty_exploration <- stringr::str_extract(
  final_response,
  "(?s)(?<=Novelty Exploration:\\*\\*\\n\\n).*?(?=\\n\\n\\*\\*Step 6)"
)

# Check if the extraction was successful
if (is.na(novelty_exploration)) {
  warning("Could not extract the Novelty Exploration section. Check the format of 'final_response' carefully.")
  novelty_exploration <- "" # Assign an empty string to avoid further errors
}

# Print the extracted content (for verification)
cat(novelty_exploration)
```
